...
# Fix joblib/loky subprocess crash on Windows when wmic is unavailable.
# Must be set before any sklearn/joblib import.
...
if page == "Home":
    ...
    retention_rate    = round(((total_customers - churned_customers) / total_customers) * 100, 1)
    ...
    with left:
        st.markdown('<h2 class="section-title">🚀 Why This Project Matters</h2>', unsafe_allow_html=True)
        st.markdown("""
...
- Retention recommendations
    ...
    with right:
        ...
        st.markdown("---")
        st.markdown("""
<div style="background:linear-gradient(135deg,#0d1f3c,#0a2a2a);padding:1.2rem;
border-radius:14px;border:1px solid #1a3a5c;">
...
<li>✔ Business Intelligence Insights</li>
    ...
    st.markdown("---")
    st.markdown('<h2 class="section-title">📈 Business Impact</h2>', unsafe_allow_html=True)
...
<h3 style='color:#00c2cb;'>📡 AI-Powered Telecom Customer Intelligence System</h3>
<p style='color:#9ca3af;'>Built with Streamlit · Plotly · Scikit-Learn · Pandas · NumPy</p>
</div>
    """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        fig_mc = px.histogram(
            df_raw, x="MonthlyCharges", nbins=50, color="Churn",
    ...
    with tab2:
        ...
        if not cat_cols:
            ...
            sel_col = st.selectbox("Select Feature", options=cat_cols, index=0, key="bivariate_selector")
            st.markdown(f"#### 📊 {sel_col} vs Churn Analysis")
            ct = df_raw.groupby([sel_col, "Churn"], dropna=False).size().reset_index(name="Count")
            fig_biv = px.bar(
                ct, x=sel_col, y="Count", color="Churn", barmode="group",
...
- **Stratified split:** 80/20
    ...
    if st.button("▶ Train Selected Model"):
        ...
        fig_gauges.update_layout(
            height=220, margin=dict(t=60, b=10, l=10, r=10),
        ...
        bar_fig = go.Figure(
            go.Bar(
                ...
                text=[f"{v:.2f}%" for v in bar_values], textposition="outside",
                cliponaxis=False,
            )
        )
        bar_fig.update_layout(
            ...
            margin=dict(t=50, b=40, l=50, r=20), height=340, showlegend=False,
        )
        ...
        with c1:
            ...
            fig_cm = go.Figure(
                go.Heatmap(
                    z=cm, text=cell_labels, texttemplate="%{text}",
            ...
            fig_cm.update_layout(
                title=f"Confusion Matrix — {selected}", height=320,
        ...
        with c2:
            if selected in ["Random Forest", "Decision Tree"]:
                st.plotly_chart(plot_feature_importance(model, feat), use_container_width=True)
            else:
    ...
    if st.button("🚀 Train ALL 5 Models"):
        ...
        st.dataframe(
            lb.style.highlight_max(subset=["Accuracy", "Precision", "Recall", "F1", "ROC-AUC"]),
            use_container_width=True,
        )
        ...
        for m in metric_cols:
            fig_percent.add_trace(
                go.Bar(
                    name=m, x=lb["Model"], y=(lb[m] * 100).round(2),
                    text=(lb[m] * 100).round(2).astype(str) + "%", textposition="outside",
                )
            )
        fig_percent.update_layout(
            ...
            plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
            height=500, legend_title="Metrics",
        )
        ...
        fig_f1 = go.Figure(
            go.Bar(
                x=lb["Model"], y=lb["F1"],
                text=[f"{v:.3f}" for v in lb["F1"]], textposition="outside",
            )
        )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<h2 class="section-title">Model Comparison Dashboard</h2>', unsafe_allow_html=True)
    ...
    if results is None:
        ...
        if all(m is not None for m in loaded.values()):
            ...
            st.info("⏳ Retraining models (saved models are missing or incompatible)...")
            trained, results = train_all_models(X_train, X_test, y_train, y_test)
    ...
    st.markdown(
        ...
        f'🏆 <strong>Champion Model: {champion}</strong><br>'
        f'F1 Score: <strong>{champ_f1:.4f}</strong> | ROC-AUC: <strong>{champ_auc:.4f}</strong><br>'
        f'<span style="font-size:0.85rem">Selected based on highest F1 score.</span>'
        f'</div>',
        unsafe_allow_html=True,
    )
    ...
    fig_cm.update_layout(
        title=f"Confusion Matrix — {best_model_name}", height=420,
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
    )
    st.plotly_chart(fig_cm, use_container_width=True)

    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "📊 Metrics Overview", "📈 ROC Curves", "🔄 PR Curves",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<h2 class="section-title">🔮 Live Churn Predictor</h2>', unsafe_allow_html=True)
    ...
    if not available or scaler_loaded is None or not feat_loaded:
            ...
            fig_gauge = go.Figure(
                go.Indicator(
                    ...
                    gauge={
                        ...
                        "threshold": {"value": 50, "line": {"color": "black", "width": 2}},
                    },
                )
            )
